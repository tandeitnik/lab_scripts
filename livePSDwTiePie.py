...
outputFolder = r"C:\Users\Labq\Dropbox\scripts\tiepie\data" #output folder where data will be saved
saveData = 0 #if 0 no data is saved, if 1 the data is saved in the outputFolder
N = 10 # número de traços
windows = 10 #number of windows used in the PSD evaluation
...
try:
    while True:
        ...
        for n in range(N):
            ...
            dataArray = np.zeros([len(data),3])
            dataArray[:,0] = np.linspace(0,acqTime,len(data)) #Time
